...
export default function ChatBubble({text, userObj, timestamp, avatar}) {
    return <Box sx={{
        ...
        {!userObj ? null : (
            <Avatar src={userObj.profile.picture} sx={{ width: 40, height: 40, marginRight: 2 }} />
        )}
        <Box sx={{
            borderRadius: "12px", 
            ...
            <Typography variant="subtitle2" fontStyle="italic" sx={{ alignSelf: 'flex-start' }}>{new Date(timestamp).toLocaleTimeString(undefined, {minute: "2-digit", hour: "2-digit"})}</Typography>
        </Box>
    </Box>
}