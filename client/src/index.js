            ...
            <Container sx={{ boxSizing: "border-box", overflow: "hidden" }} maxWidth="false">
              ...
              <Container sx={{ boxSizing: "border-box", overflow: "hidden" }} maxWidth="false">
                <Box sx={{
                    ...
                    {/*selectedArtists.length>0 && <ArtistProfile artist={selectedArtists}/>*/ }
                    <Routes>
                      <Route path="/groupList/:id?" element={<GroupList groups={groups} />} />
                      <Route path="/artistProfile/:id?" element={<ArtistProfile />} />
                      <Route exact path="/artistList" element={<ArtistListWithProfile groups={groups} />} />
                    </Routes>
                </Box>
              </Container>
            </Container>
...
function ArtistListWithProfile({ groups }) {
  return (
    <Box sx={{ position: 'relative' }}>
      <Routes>
        <Route path="/artistList/:id" element={<ArtistProfile />} />
      </Routes>
      <ArtistList groups={groups} />
    </Box>
  );
}
